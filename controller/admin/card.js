...
const card = {
     ...
     listData: async function (req, res) {
          ...
          try {
               ...
               let cnt = await models.cardModel.count({
                    where
               })
               let rows = await models.cardModel.findAll({
                    where,
                    limit: limit,
                    offset: offset,
                    order: [['id', 'asc']]
               ...
               for (let i in rows) {
                    let needCard = rows.find(r => r.id == rows[i].cardId)
                    if (needCard) {
                         rows[i].dataValues.needCard = needCard.name;
                    } else {
                         rows[i].dataValues.needCard = '不限'
                    }
               }
