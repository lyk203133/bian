...
(async ()=>{
  let resp = await getPricing();//axios.get('https://api.jz1378.com/pricing')
  ...
  <div class="list">
    ...
    <van-row justify="left"  v-for="(row,index) in rows">
      <van-col span="3" class="item">
        <img class="icon" :src="`./assets/images/${row.currencyName.toLowerCase()}.png`" />
      </van-col>
      <van-col span="4" class="">{{ row.currencyName }}</van-col>
      <van-col span="8" class="">${{ row.lastPrice }}</van-col>
      <van-col span="8" class="">
        <svg width="100" height="40" :id="'chart'+(index+1)"></svg>
      </van-col>
    </van-row>
